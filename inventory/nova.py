...
def nova_load_config_file():
    ...
    path4 = os.path.dirname(os.path.realpath(__file__)) + "/nova.ini"
...
if len(sys.argv) == 2 and (sys.argv[1] == '--list'):
	...
	if f.accessIPv4:
        	#groups[group].append(f.name.join(' ansible_ssh_host=').join(f.accessIPv4))
        	groups[group].append(f.name)
		continue
	if public:
        	#groups[group].append(f.name.join(' ansible_ssh_host=').join(public))
                groups[group].append(f.name)
		continue
	if private:
        	#groups[group].append(f.name.join(' ansible_ssh_host=').join(private))
                groups[group].append(f.name)
		continue
...
# variables for that host
	...
	if sys.argv[2] == instance.name:
            ...
            if public != []:
                results['ansible_ssh_host'] = ''.join(public)
            else:
                results['ansible_ssh_host'] = ''.join(private)
